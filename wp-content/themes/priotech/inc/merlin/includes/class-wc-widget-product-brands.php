 ...
 * @extends WC_Widget
 */
class Priotech_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'priotech');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'priotech');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'priotech'),
				'label' => esc_html__('Title', 'priotech'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'priotech'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'priotech'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'priotech'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'priotech'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'priotech'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'priotech'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'priotech'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Priotech_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'priotech');
			...
			echo '<ul class="product-brands" id="priotech-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Priotech_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'priotech'),
		'singular_name'              => esc_html__('Brands', 'priotech'),
		'menu_name'                  => esc_html__('Brands', 'priotech'),
		'all_items'                  => esc_html__('All Brands', 'priotech'),
		'parent_item'                => esc_html__('Parent Brand', 'priotech'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'priotech'),
		'new_item_name'              => esc_html__('New Brand Name', 'priotech'),
		'add_new_item'               => esc_html__('Add New Brands', 'priotech'),
		'edit_item'                  => esc_html__('Edit Brand', 'priotech'),
		'update_item'                => esc_html__('Update Brand', 'priotech'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'priotech'),
		'search_items'               => esc_html__('Search Brands', 'priotech'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'priotech'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'priotech'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
