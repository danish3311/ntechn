...
class Priotech_Elementor_Sticky_Section {
    ...
    public function scripts() {
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/frontend/jquery.sticky.js', ['jquery'], $priotech_version, true);
        wp_enqueue_script('priotech-elementor-sticky', get_template_directory_uri() . '/assets/js/libs/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], $priotech_version, true);
    }
    ...
    public function register_controls(Element_Base $element) {
        $element->add_control(
            'sticky',
            [
                'label'              => esc_html__('Sticky', 'priotech'),
                'type'               => Controls_Manager::SELECT,
                'options'            => [
                    ''       => esc_html__('None', 'priotech'),
                    'top'    => esc_html__('Top', 'priotech'),
                    'bottom' => esc_html__('Bottom', 'priotech'),
                ],
                'separator'          => 'before',
                'render_type'        => 'none',
        ...
        foreach ($active_devices as $device) {
            $label                          = 'desktop' === $device ? esc_html__('Desktop', 'priotech') : $active_breakpoint_instances[$device]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label'              => esc_html__('Sticky On', 'priotech'),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label'              => esc_html__('Offset', 'priotech'),
        ...
        $element->add_responsive_control(
            ...
            [
                'label'              => esc_html__('Effects Offset', 'priotech'),
                'type'               => Controls_Manager::NUMBER,
                'default'            => 0,
                'min'                => 0,
                'max'                => 1000,
        ...
        if ($this->is_instance_of($element, $types)) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label'              => esc_html__('Stay In Column', 'priotech'),
...
new Priotech_Elementor_Sticky_Section();
