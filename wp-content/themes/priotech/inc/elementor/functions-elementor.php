...
if (!function_exists('priotech_elementor_parse_text_editor')) {
    function priotech_elementor_parse_text_editor($content, $obj) {
...
if (!function_exists('priotech_elementor_get_strftime')) {
    function priotech_elementor_get_strftime($instance, $obj) {
        $string = '';
...
if (!function_exists('priotech_elementor_breakpoints')) {
    function priotech_elementor_breakpoints() {
        ...
        wp_add_inline_style('priotech-style', $var);
    }
}

function priotech_get_hotspots() {
    global $post;

    $options[''] = esc_html__('Select Hotspots', 'priotech');
...
function priotech_get_file_contents($path) {
    if (is_file($path)) {
...
function priotech_get_icon_svg($path, $color = '', $width = '') {
    $content = priotech_get_file_contents($path);
    if ($content) {
        $re = '/<svg(([^\n]*\n)+)<\/svg>/';
