...
class Priotech_Header_Footer_Elementor {
    public function __construct() {
        add_action('wp', [$this, 'hooks'], 99999);

        add_filter( 'walker_nav_menu_start_el', [$this, 'change_class_icon_dropdown_header'], 10, 4);
    }
    ...
    public function change_class_icon_dropdown_header($item_output, $item, $depth, $args) {
        if (isset($args->menu_class) && $args->menu_class == 'hfe-nav-menu') {
            $item_output = str_replace("<i class='fa'></i>","<span class='priotech-icon-angle-right'></span>",$item_output);
        }
        return $item_output;
    }
}

new Priotech_Header_Footer_Elementor();
