(function ($) {
    ...
    function woo_dropdown_sidebar() {
        ...
        $body.on('click', '.widget.priotech-widget-woocommerce .priotech_title_filter',function (e) {
            e.preventDefault();
            var widgetTitle = $(this).parent('.widget-title');
            var $parent = widgetTitle.closest('.priotech-menu-filter');
            if($parent.length){
                ...
                //     .parent('.widget').find('.priotech_title_filter').removeClass('toggled-on');
    ...
    function woo_drawing_sidebar() {
        ...
        if($body.hasClass('priotech-drawing-side') && sidebarDrawing.length > 0) {
            $body.on('click', '.sidebar-drawing-toggle > a',function (e) {
                e.preventDefault();
                $(this).toggleClass('actived');
                
                $body.toggleClass('priotech-sidebar-left');
    ...
    function sendRequest(url, append = false) {
        ...
        xhr = $.ajax({
            ...
            url: url,
            beforeSend: function () {
                var $products = $('ul.priotech-products');
                if(!append) {
            ...
            success: function (data) {
                let $html = $(data);
                if(append) {
                    $('#main ul.priotech-products').append($html.find('#main ul.priotech-products > li'));
                }else {
                    $('#main ul.priotech-products').replaceWith($html.find('#main ul.priotech-products'));
                }
                $('#main .woocommerce-pagination-wrap').replaceWith($html.find('#main .woocommerce-pagination-wrap'));
                window.history.pushState(null, null, url);
                xhr = false;
                $(document).trigger('priotech-products-loaded');
            }
        });
    }

    $body.on('change', '.priotech-products-per-page #per_page', function (e) {
        e.preventDefault();
        var url = this.value;
        sendRequest(url);
    });
    ...
    $(document).ready(function () {
        ...
        woo_drawing_sidebar();
    }).on('priotech-products-loaded',function () {
